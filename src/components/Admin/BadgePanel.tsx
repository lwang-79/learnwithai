...
function BadgePanel() {
  ...
  const addCriteriaButtonClickedHandler = () => {
    ...
    setCriteria([
      ...criteria,
      `${formState.criteriaName}::${formState.criteriaValue}`,
    ]);
  };
  ...
  const toggleVisibility = async (index: number) => {
    await DataStore.save(
      Badge.copyOf(badges[index], (badge) => {
    ...
    <VStack w="full" spacing={4}>
      <Card w="full">
        <VStack px={4} py={4}>
            ...
            <VStack w="full">
              ...
              <HStack w="full">
                ...
                <FormControl display="flex" alignItems="center" ps={4}>
                  ...
                  <Switch
                    onChange={setInput("isVisible")}
                    colorScheme="teal"
                    isChecked={formState.isVisible}
                  />
                </FormControl>
              </HStack>
            </VStack>
          ...
          <HStack w="full">
            {criteria.length === 0 && (
              <Text ms={4}>{`Click button to add criteria   ☝️`}</Text>
            )}
            {criteria.map((c, index) => (
              <Tag key={`${c}_${index}`} rounded="full">
                <TagLabel>{c}</TagLabel>
                <TagCloseButton
                  onClick={() => removeCriteriaButtonClickedHandler(index)}
                />
              </Tag>
            ))}
            ...
            <Button w="50%" variant="ghost" onClick={addBadgeClickedHandler}>
              Add Badge
            </Button>
          </HStack>
        </VStack>
          ...
          <Card key={badge.id} w="full" p={4}>
            <HStack w="full">
              ...
              <VStack w="full" align="flex-start">
                ...
                <Text fontSize="xs">{badge.description}</Text>
              </VStack>
            </HStack>
          </Card>
        ))}
