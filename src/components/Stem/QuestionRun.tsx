...
function QuestionRun({ level, concepts, mode, initMaxNum = defaultNumber, onClose }: QuestionRunProps) {
  ...
  useEffect(() => {
    const getAndSetStemQuestions = async (num: number) => {
      const questionSets = await getStemQuestionsFromDataset(apiName, concepts, level, num);
        ...
        <>
          ...
          >
            <HStack w='full'>
              ...
              <Spacer />
              {(isTest) && <Timer isStopped={timerStopped} duration={duration} setDuration={setDuration} />}
            </HStack>
              ...
              >
                ...
                <Button
                  ...
                  isDisabled={
                    ...
                    (currentIndexRef.current >= lastIndexRef.current && (isSubmitted)) ||
