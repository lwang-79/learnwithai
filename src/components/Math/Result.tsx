...
function Result({ questionSets }: ResultProps) {
  ...
  for (const question of questionSets) {
    ...
    const existingConcept = resultsByConcept.find(
      (concept) => concept.concept === question.concept,
    );
    ...
    if (existingConcept) {
      ...
      resultsByConcept.push({
        concept: question.concept,
        total: 1,
        correct: question.answer === question.selected ? 1 : 0,
      });
    }
  }
  ...
  return (
    <VStack mt={6}>
      ...
      <HStack w="full">
        ...
        <Spacer />
        <Text as="b">
          {correct}/{questionSets.length}
        </Text>
      </HStack>
      {resultsByConcept.map((result, index) => (
        <HStack key={`${result.concept}-${index}`} w="full">
          <Text w="30%">
            {result.concept.charAt(0).toUpperCase() + result.concept.slice(1)}
          </Text>
