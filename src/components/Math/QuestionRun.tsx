...
function QuestionRun({
  ...
  const { dataStoreUser, setDataStoreUser, setIsProcessing, apiName } =
  ...
  const testRef = useRef<TestState | undefined>(
    initialTest ? (initialTest as TestState) : undefined,
  );
  ...
  useEffect(() => {
    ...
    if (source.includes("ChatGPT") || source === QuestionSource.Hendrycks) {
  ...
  useEffect(() => {
    ...
    if (questionSets.length < lastIndexRef.current + 1) {
      setQuestionSets(
        questionSetsRef.current.slice(0, lastIndexRef.current + 1),
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetchingStatus]);
  ...
  const addSavedQuestionSets = async () => {
    ...
    savedQuestionSetsRef.current = allSavedQuestionSets.sort((a, b) =>
    ...
    for (const qs of savedQuestionSetsRef.current) {
      ...
      questionSetsRef.current = [...questionSetsRef.current, localQS];
    }
  ...
  const getAndSetDatasetQuestions = async (
    num: number,
    concept?: HendrycksConcept,
  ) => {
    const questionSets = await getQuestionsFromDataset(
      apiName,
      source,
      num,
      level,
  ...
  const nextButtonClickedHandler = async () => {
    ...
    if (currentIndexRef.current > lastIndexRef.current && !isSubmitted) {
      ...
      if (
        questionSetsRef.current.length + addingQuestionCountRef.current <
        maxNum
      ) {
...
Correct: ${correct} (${((100 * correct) / (lastIndexRef.current + 1)).toFixed(0) + "%"})
  ...
  const closeButtonClickedHandler = async () => {
    ...
    if (!isSavedQuestions) {
      // Saved question doesn't consume quota
  ...
  const deleteButtonClickedHandler = async () => {
    if (
      !savedQuestionSetsRef.current ||
      savedQuestionSetsRef.current.length === 0
    ) {
      return;
    }

    try {
      const { testId, indexInTest } =
        savedQuestionSetsRef.current[currentIndexRef.current];
  ...
  return (
    <>
      <IconButton
        ...
        onClick={
          isTest && !isSubmitted ? onOpenExitAlert : closeButtonClickedHandler
        }
        zIndex={100}
        position="fixed"
        top={2}
            ...
            >
              <VStack align="flex-start" spacing={2} w="full" pe={4}>
                <HStack w="full">
                  ...
                  {currentQuestionSet && currentQuestionSet.concept && (
                    <Tag rounded="full" fontSize="xs">
                      {currentQuestionSet.concept.charAt(0).toUpperCase() +
                        currentQuestionSet.concept.slice(1)}
                    </Tag>
                  )}
                  <Spacer />
                    ...
                    <>
                      ...
                      >
                        <IconButton
                          ...
                          isDisabled={
                            !currentQuestionSet || isReview || isSubmitted
                          }
                        />
                      </Tooltip>
                      ...
                      {(source.includes("ChatGPT") ||
                        ...
                        dataStoreUser!.membership!.current > 2 && (
                          <Tooltip hasArrow label="Report bad question">
                            <IconButton
                              rounded="full"
                              variant={
                ...
                <Button
                  ...
                  isDisabled={
                    (!value && !isSavedQuestions) ||
                    !currentQuestionSet ||
                    (currentIndexRef.current >= lastIndexRef.current &&
                      (isSubmitted || isSavedQuestions)) ||
              ...
              >
                ...
                <Button
                  onClick={onOpenAlert}
                  isDisabled={
                    questionSets.length === 0 || isSubmitted || isReview
                    // addingQuestionCountRef.current !== 0
                  }
                >
                  Submit
          ...
          >
            <AlertDialogOverlay>
              <AlertDialogContent>
                ...
                <AlertDialogBody>
                  You have not submitted your answers. Are you sure you want to
                  exit?
                </AlertDialogBody>
