...
export const getDataStoreUserOrCreateIfNotExist = async (
  userAttributes: any,
...
const retryGetDataStoreUserById = async (
  userId: string,
): Promise<User | undefined> => {
  let retryCount = 0;
...
export const getLevel = (
  daily: Statistic[],
): { mathLevel: number; writingLevel: number } => {
  let mathLevel = 0;
  let writingLevel = 0;

  const dateObj = new Date();
  dateObj.setDate(dateObj.getDate() - 7);
  const weeklyData = daily.filter(
    (stat) => stat.date >= dateObj.toLocaleString("sv-SE").slice(0, 10),
  );

  const mathTotal = weeklyData.reduce(
    (acc, { mathCorrect }) => acc + mathCorrect,
    0,
  );
  const writingTotal = weeklyData.reduce(
    (acc, { writing }) => acc + writing,
    0,
  );
