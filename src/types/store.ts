...
const redeemReward = async (user: User, reward: ShoppingItem) => {
  if (user && user.notification && user.notification.emails.length > 0) {
...
const redeemSeed = async (user: User, seed: ShoppingItem) => {
  ...
  if (user && user.gameData) {
    ...
    if (random <= seed.price / 1000) {
      ...
      randomIndex =
        Math.floor(Math.random() * (SEEDS.length - midpoint)) + midpoint;
    } else {
      const midpoint = Math.floor(SEEDS.length / 2);
  ...
  const updatedUser = await DataStore.save(
    User.copyOf(user, (updated) => {
      updated.gameData!.startDate = new Date()
...
export { redeemReward, redeemSeed };
