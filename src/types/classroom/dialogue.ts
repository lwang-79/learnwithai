...
const getDialogue = async (messages: ChatMessage[], apiName: APIName) => {
  ...
  if (!words)  { throw new Error('No words returned'); }
...
const getNextRole = (
  ...
  if (nextRole) {
    const character = characters.find(character => character.role === nextRole || character.name === nextRole);
    if (character) {
      return `${character.role}-${character.name}`;
    }
  }
  const nonTeacherCharacters = characters.filter(character => character.role !== 'teacher');

  if (currentRole.includes('teacher')) {
    const randomNonTeacherCharacter = nonTeacherCharacters[Math.floor(Math.random() * nonTeacherCharacters.length)];
  ...
  if (random < 0.8) {
    const teacherCharacter = characters.find(character => character.role === 'teacher')!;
    return `${teacherCharacter.role}-${teacherCharacter.name}`;
  }

  const nonTeacherAndCurrentCharacters = nonTeacherCharacters.filter(character => character.name !== currentRole.split('-')[1]);

  const randomNonTeacherCharacter = nonTeacherAndCurrentCharacters[Math.floor(Math.random() * nonTeacherAndCurrentCharacters.length)];
