...
function Classroom() {
  ...
  useEffect(() => {
    ...
    if (dialoguesRef.current.length > length) {
      ...
      if (isAudioEnabled) {
        ...
        const voice = charactersRef.current.find(
          (character) => character.name === newDialogues[0].role.split("-")[1],
        )?.voice;
        playSpeech(newDialogues[0].content, voice).then(() => {
          setIsAudioPlaying(false);
          setTimeout(() => {
            setAudioPlayToggle.toggle();
          }, 1000);
        });
      }
    }
  ...
  const startButtonClickedHandler = async () => {
    ...
    while (isPlayingRef.current && roundRef.current < maxRoundRef.current) {
      let words = "";
      let nextRole = "";
      let tryCount = 0;
      ...
      do {
        try {
          tryCount++;
          ({ words, nextRole } = await getDialogue(
            messagesRef.current,
            apiName,
          ));
        } catch (error) {
        ...
        <VStack spacing={1}>
          <HStack w="full">
            ...
            <InputGroup size="sm" maxW="130px">
              ...
              {dataStoreUser && (
                <NumberInput
                  ...
                  isDisabled={!isPlayingFinished}
                  min={5}
                  max={dataStoreUser.quota!.classroomRound}
                  step={5}
            ...
            {isPlaying ? (
              ...
              <Tooltip
                label={isPlayingFinished ? "Start" : "Continue"}
                hasArrow
              >
                <IconButton
                  rounded="full"
            ...
            {dataStoreUser && isAudioEnabled ? (
              ...
              dataStoreUser && (
                <Tooltip
                  label={
                    dataStoreUser.membership!.current > 1
                      ? "Unmute from next"
                      : "No available for your current plan"
                  }
                  hasArrow
                >
                  <IconButton
        ...
        {!isPlayingFinished || dialogues.length > 0 ? (
          <>
            {dialogues.length > 0 ? (
              <>
                {dialogues.map((dialogue, index) => (
                  <HStack key={index} w="full">
                    {dialogue.role.split("-")[0] === "teacher" ||
                    dialogue.role.split("-")[0] === "System" ? (
                      <>
                        <Tooltip label={dialogue.role.split("-")[1]} hasArrow>
                          <HStack spacing={0}>
                            <Avatar
                              ...
                              name={dialogue.role.split("-")[1]}
                              size="sm"
                              mr={2}
                              src={getPicture(dialogue.role.split("-")[1])}
                            />
                            {index === 0 && isAudioPlaying && (
                              <Player
                                src="/classroom/sound.json"
                                autoplay
                                loop
                      ...
                      <>
                        <Tag p={2} w="full" colorScheme="blue">
                          <Text as="span">{dialogue.content}</Text>
                        </Tag>
                        <Tooltip label={dialogue.role.split("-")[1]} hasArrow>
                          <HStack>
                            {index === 0 && isAudioPlaying && (
              ...
              <Tag p={4} w="full" colorScheme="blue">
