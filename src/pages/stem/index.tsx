...
function Stem() {
  ...
  const setAllCheckedConcepts = () => {
    if (selectedConcepts.length === 0) {
      if (selectedLevel === 'High School') {
        const highSchoolConcepts = concepts.filter(concept => concept !== StemConcept.Medicine && concept !== StemConcept.Electrical);
        setSelectedConcepts([...highSchoolConcepts]);
      } else {
        const collegeConcepts = concepts.filter(concept => concept !== StemConcept.Statistics);
        setSelectedConcepts([...collegeConcepts]);
      }
    } else {
      setSelectedConcepts([]);
