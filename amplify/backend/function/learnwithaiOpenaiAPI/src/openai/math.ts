        ...
        : ds === QuestionSource.Competition
          ...
          : ds === QuestionSource.Hendrycks
            ? (hendrycks as HendrycksQuestion[]).filter(
                (question) =>
                  question.level === level && concept === question.type,
              )
            : [];
...
const generateHendrycksQuestionSets = async (
  ...
  for (const question of questions) {
    ...
    while (count < 3 && !done) {
      try {
        const response = await generateMathOptions(
          question.problem,
          question.solution,
        );
