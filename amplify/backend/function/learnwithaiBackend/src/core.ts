import { createRankingItem, getRankingItemsByDateAndType, listUserData, RankingItem, RankingType, updateRankingItem, UserData } from "./graphql"
...
async function getStaticDataAndSendNotification(users: UserData[], dayString: string, monthString: string) {
  ...
  for (const user of users) {
    if (user.daily) {
      ...
      if (dayData) {
        const mathScore = dayData.mathExam * 20 + dayData.mathCorrect * 10 + dayData.mathWrong;
        if (mathScore > 100) {
    ...
    if (user.monthly) {
      ...
      if (monthData) {
        const mathScore = monthData.mathExam * 20 + monthData.mathCorrect * 10 + monthData.mathWrong;
        if (mathScore > 500) {
...
async function processData() {
  ...
  const dayString = new Date(yesterday.toLocaleString('sv').slice(0,10)).toISOString().slice(0,10);
  ...
  await createOrSaveRankingItem(RankingType.MATH_CORRECT_NUMBER_BY_DAY, dayString, mathCorrectNumberByDayData);
  await createOrSaveRankingItem(RankingType.MATH_CORRECT_NUMBER_BY_MONTH, monthString, mathCorrectNumberByMonthData);
  await createOrSaveRankingItem(RankingType.WRITING_NUMBER_BY_DAY, dayString, writingNumberByDayData);
  await createOrSaveRankingItem(RankingType.WRITING_NUMBER_BY_MONTH, monthString, writingNumberByMonthData);
...
async function createOrSaveRankingItem(
  ...
  while (data.length < 5) {
    ...
    const randomScore = type === RankingType.MATH_CORRECT_NUMBER_BY_DAY ?
